...
type OutlierDetectionInformer interface {
	...
	Lister() v1.OutlierDetectionLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.AdmiralV1().OutlierDetections(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AdmiralV1().OutlierDetections(namespace).Watch(context.TODO(), options)
			},
		},
		&admiralv1.OutlierDetection{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *outlierDetectionInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&admiralv1.OutlierDetection{}, f.defaultInformer)
}

func (f *outlierDetectionInformer) Lister() v1.OutlierDetectionLister {
	return v1.NewOutlierDetectionLister(f.Informer().GetIndexer())
}
