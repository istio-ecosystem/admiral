...
type OutlierDetectionLister interface {
	...
	List(selector labels.Selector) (ret []*v1.OutlierDetection, err error)
...
// List lists all OutlierDetections in the indexer.
func (s *outlierDetectionLister) List(selector labels.Selector) (ret []*v1.OutlierDetection, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.OutlierDetection))
	})
	return ret, err
}
...
type OutlierDetectionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.OutlierDetection, error)
...
// List lists all OutlierDetections in the indexer for a given namespace.
func (s outlierDetectionNamespaceLister) List(selector labels.Selector) (ret []*v1.OutlierDetection, err error) {
...
// Get retrieves the OutlierDetection from the indexer for a given namespace and name.
func (s outlierDetectionNamespaceLister) Get(name string) (*v1.OutlierDetection, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("outlierdetection"), name)
	}
	return obj.(*v1.OutlierDetection), nil
}
